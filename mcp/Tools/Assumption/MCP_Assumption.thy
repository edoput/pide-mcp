...
theory MCP_Assumption
  ...
  keywords
    "spec_test" :: thy_decl
    and "verifies" "discharges"
begin

ML \<open>
signature MCP_ASSUMPTION =
sig
  datatype relation = Verifies | Discharges
...
fun relation_name Verifies = "verifies"
  | relation_name Discharges = "discharges";
...
fun check_link relation arg =
  let
    ...
    val well_typed =
      (case relation of Discharges => test_obligation | Verifies => not test_obligation);
    val _ = well_typed orelse
      error (relation_name relation ^ " cannot target " ^ quote id ^
        (if test_obligation then "; T labels must be discharged"
         else "; A/I/D/Q labels must be verified") ^ Position.here (#2 arg));
...
fun declare (name, pos) verifies discharges thy =
  let
    val _ =
      if exists (not o Char.isSpace) (String.explode name) then ()
      else error ("spec_test name must not be empty" ^ Position.here pos);
    val links = map (check_link Verifies) verifies @ map (check_link Discharges) discharges;
    val _ =
      if null links then
        error ("spec_test needs at least one verifies or discharges link" ^ Position.here pos)
      else ();
...
val _ =
  Outer_Syntax.command \<^command_keyword>\<open>spec_test\<close>
    ...
    (Parse.embedded_position --
      ...
      Scan.optional
        (\<^keyword>\<open>discharges\<close> |-- Parse.and_list1 Parse.embedded_position) [] >>
      (fn ((name, verifies), discharges) =>
        Toplevel.theory (declare name verifies discharges)));
